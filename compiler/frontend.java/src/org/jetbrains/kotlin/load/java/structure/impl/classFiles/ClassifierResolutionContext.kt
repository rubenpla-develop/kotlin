...
class ClassifierResolutionContext private constructor(
    ...
    data class Result(val classifier: JavaClassifier?, val qualifiedName: String)

    private class InnerClassInfo(val outerInternalName: String, val simpleName: String)

    fun addInnerClass(innerInternalName: String, outerInternalName: String, simpleName: String) {
    ...
    fun addTypeParameters(newTypeParameters: Collection<JavaTypeParameter>) {
    ...
    fun resolveClass(classId: ClassId) = Result(classesByQName(classId), classId.asSingleFqName().asString())
    fun resolveTypeParameter(name: String) = Result(typeParameters?.get(name), name)

    fun copyForMember() = ClassifierResolutionContext(classesByQName, typeParameters?.let(::THashMap), innerClasses?.let(::THashMap))

    // See com.intellij.psi.impl.compiled.StubBuildingVisitor.createMapping(byte[])
    fun mapInternalNameToClassId(internalName: String): ClassId {
        if ('.' in internalName) {
    ...
    fun resolveByInternalName(c: String) = resolveClass(mapInternalNameToClassId(c))
}
