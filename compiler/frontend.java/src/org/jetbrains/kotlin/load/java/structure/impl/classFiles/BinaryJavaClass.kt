...
class BinaryJavaClass(
    ...
    override fun visitMethod(access: Int, name: String, desc: String, signature: String?, exceptions: Array<out String>?): MethodVisitor? {
        ...
        val (member, visitor) = BinaryJavaMethodBase.create(name, access, desc, signature, this, context.copyForMember(), signatureParsingComponent)
    ...
    private fun parseClassSignature(signature: String) {
        ...
        supertypes.addIfNotNull(signatureParsingComponent.parseTopLevelClassRefSignature(iterator, context))
    ...
    override fun visitField(access: Int, name: String, desc: String, signature: String?, value: Any?): FieldVisitor? {
        ...
        val type = signatureParsingComponent.parseTypeString(StringCharacterIterator(signature ?: desc), context)
        ...
        return BinaryJavaField(Name.identifier(name), access, this, access.isSet(Opcodes.ACC_ENUM), type, processedValue).run {
            ...
            object : FieldVisitor(ASM_API_VERSION_FOR_CLASS_READING) {
                override fun visitAnnotation(desc: String, visible: Boolean) =
                        BinaryJavaAnnotation.addAnnotation(this@run.annotations, desc, context, signatureParsingComponent)
            }
        }
    }
    ...
    override fun visitAnnotation(desc: String, visible: Boolean) =
            BinaryJavaAnnotation.addAnnotation(annotations, desc, context, signatureParsingComponent)
    ...
    override fun findInnerClass(name: Name): JavaClass? {
        ...
        return virtualFile.parent.findChild("${virtualFile.nameWithoutExtension}$$name.class")?.let {
            BinaryJavaClass(it, fqName.child(name), context.copyForMember(), signatureParsingComponent, access, this)
        }
    }
