 ...
 */
class SignatureParsingComponent(globalContext: ClassifierResolutionContext) {
    companion object {
    ...
    private fun parseTypeParameter(signature: CharacterIterator, context: ClassifierResolutionContext): JavaTypeParameter {
        ...
        while (signature.current() == ':') {
            signature.next()
            val bound = parseTopLevelClassRefSignature(signature, context) ?: continue
            if (bounds == null) {
    ...
    fun parseTopLevelClassRefSignature(signature: CharacterIterator, context: ClassifierResolutionContext): JavaClassifierType? {
