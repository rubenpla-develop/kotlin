...
class KotlinEvaluator(val codeFragment: KtCodeFragment, val sourcePosition: SourcePosition) : Evaluator {
    override fun evaluate(context: EvaluationContextImpl): Any? {
        ...
        try {
            ...
            if (compiledData.classes.isNotEmpty()) {
                loadClasses(context, compiledData.classes)
            }

            val result = evaluateWithCompilation(context, compiledData) ?: runEval4j(context, compiledData)
    ...
    companion object {
        private fun extractAndCompile(codeFragment: KtCodeFragment, sourcePosition: SourcePosition, context: EvaluationContextImpl): CompiledDataDescriptor {
            ...
            val additionalFiles = outputFiles.map { getClassName(it.relativePath) to it.asByteArray() }
        ...
        private fun evaluateWithCompilation(context: EvaluationContextImpl, compiledData: CompiledDataDescriptor): Any? {
            ...
            val mainClassBytecode = compiledData.classes[0].second
            ...
            val mainClass = context.classLoader?.visibleClasses()?.firstOrNull { clazz ->
                clazz.name() == mainClassJdiName
            ...
            try {
                ...
                if (!mainClass.isPrepared) {
                    val javaLangClass = vm.classesByName(Class::class.java.name).firstOrNull() as? ClassType
                                        ?: error("Can't resolve java.lang.Class")
                    val forNameMethod = javaLangClass.methodsByName(
                            "forName", "(Ljava/lang/String;ZLjava/lang/ClassLoader;)Ljava/lang/Class;").firstOrNull()
                            ?: error("Can't find 'forName' method")

                    javaLangClass.invokeMethod(
                            thread,
                            forNameMethod,
                            listOf(vm.mirrorOf(mainClassJdiName), vm.mirrorOf(true), classLoader),
                            invokePolicy)
                }
                ...
                return mainClass.invokeMethod(thread, mainClass.methods().single(), args, invokePolicy)
            } catch (e: Throwable) {
